...
public class RectangleFormatController {
    ...
    public static void Import(String[] c) {
        rectangle = c;
        DataBase.rootRectangle = build(1, 1, c.length - 1, c[0].length() - 1);
    }
    ...
    static Node build(int x1, int y1, int x2, int y2) {
        ...
        for (int i = x1; i < x2; i++) {
            ...
            if (tmp) {
                ...
                node.width = (y2 - y1 + 1);
                node.rightChild = build(i + 1, y1, x2, y2);
                node.leftChild = build(x1, y1, i, y2);
                node.height = node.rightChild.height + node.leftChild.height;
                return node;
            }
        }
        for (int j = y1; j < y2; j++) {
            ...
            if (tmp) {
                node.value = String.valueOf('|');
                node.height = (x2 - x1 + 1);
                node.rightChild = build(x1, j + 1, x2, y2);
                node.leftChild = build(x1, y1, x2, j);
                node.width = node.rightChild.width + node.leftChild.width;
                return node;
            }
        }
        node.value = "";
    ...
    public static void Export(Node node) {
        ...
        rec = new char[DataBase.rootRectangle.height + 1][DataBase.rootRectangle.width + 1];
        for (int i = 0; i < DataBase.rootRectangle.height + 1; i++)
            for (int j = 0; j < DataBase.rootRectangle.width + 1; j++)
                rec[i][j] = ' ';
        for (int i = 0; i < DataBase.rootRectangle.height + 1; i++)
            rec[i][0] = rec[i][DataBase.rootRectangle.width] = '|';
        for (int i = 1; i < DataBase.rootRectangle.width; i++)
            rec[0][i] = rec[DataBase.rootRectangle.height][i] = '-';
    ...
    static void buildRec(int x, int y, Node node) {
        if (!node.value.equals("-") && !node.value.equals("|")) {
            ...
            for (int i = x; i < x + node.width - 1; i++)
                for (int j = y; j < y + node.height - 1 && in < node.value.length(); j++, in++)
                    if (rec[i][j] == ' ')
        ...
        if (node.value.equals("|")) {
            for (int i = x; i < x + node.height - 1; i++)
                rec[i][y + node.leftChild.width - 1] = '|';
            // print();
            buildRec(x, y, node.leftChild);
            buildRec(x, y + node.leftChild.width, node.rightChild);
        } else {
            for (int i = y; i < y + node.width - 1; i++)
                rec[x + node.leftChild.height - 1][i] = '-';
            // print();
            buildRec(x, y, node.leftChild);
            buildRec(x + node.leftChild.height, y, node.rightChild);
        }
    }
